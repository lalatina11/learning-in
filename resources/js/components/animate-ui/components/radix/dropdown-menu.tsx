...
function DropdownMenuContent({ sideOffset = 4, className, children, ...props }: DropdownMenuContentProps) {
    return (
        <DropdownMenuContentPrimitive
            ...
            {...props}
        >
            <DropdownMenuHighlightPrimitive className="absolute inset-0 z-0 rounded-sm bg-accent">{children}</DropdownMenuHighlightPrimitive>
        </DropdownMenuContentPrimitive>
    );
}
            ...
            >
                <span className="pointer-events-none absolute left-2 flex size-3.5 items-center justify-center">
                    <DropdownMenuItemIndicatorPrimitive initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }}>
                        {/* <UnreadLineDuotoneIcon className="size-4" /> */}
            ...
            >
                <span className="pointer-events-none absolute left-2 flex size-3.5 items-center justify-center">
                    <DropdownMenuItemIndicatorPrimitive layoutId="dropdown-menu-item-indicator-radio">
                        {/* <RecordLineDuotoneIcon className="size-2 fill-current" /> */}
                    </DropdownMenuItemIndicatorPrimitive>
                </span>
                {children}
            </DropdownMenuRadioItemPrimitive>
...
function DropdownMenuLabel({ className, inset, ...props }: DropdownMenuLabelProps) {
    return (
        <DropdownMenuLabelPrimitive data-inset={inset} className={cn('px-2 py-1.5 text-sm font-medium data-[inset]:pl-8', className)} {...props} />
    );
}
...
function DropdownMenuShortcut({ className, ...props }: DropdownMenuShortcutProps) {
    return <DropdownMenuShortcutPrimitive className={cn('ml-auto text-xs tracking-widest text-muted-foreground', className)} {...props} />;
}
...
function DropdownMenuSubTrigger({ className, inset, children, ...props }: DropdownMenuSubTriggerProps) {
    return (
        <DropdownMenuHighlightItemPrimitive>
            ...
            >
                {children}
                {/* <AltArrowRightLineDuotoneIcon data-slot="chevron" className="ml-auto size-4" /> */}
            </DropdownMenuSubTriggerPrimitive>
        </DropdownMenuHighlightItemPrimitive>
    );
}
